...
tab) -- trimmed to only the pieces dd_idea actually uses.
...
`Receptor`/`Pose` I/O (`io.py`) -- none of which dd_idea needs, and the
first of which imports `rdkit` at module level. dd_idea has no
...
Streamlit component (`component.py`) are vendored -- importing `dd_idea.viewer3d`
...
def __getattr__(name):
    ...
    # `dd_idea.viewer3d` at all (e.g. from `pipeline.py`, which never touches
